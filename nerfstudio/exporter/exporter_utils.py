...
def generate_point_cloud(
    ...
    with progress as progress_bar:
        ...
        while not progress_bar.finished:
            ...
            true_indices = outputs["true_indices"]
            ...
            selected_origins = torch.index_select(ray_bundle.origins, dim=0, index=true_indices.squeeze(1))
            selected_directions = torch.index_select(ray_bundle.directions, dim=0, index=true_indices.squeeze(1))
            #CONSOLE.print(f"origins.shape: {selected_origins.shape}, directions.shape: {selected_directions.shape}")
            point = selected_origins + selected_directions * depth
            view_direction = selected_directions
